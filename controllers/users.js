...
class userControllers {
    ...
    static async createUser(req, res) {
        ...
        const { email, motDePasse, confirmeMotDePasse, name, prenom, ...body } = req.body
        console.log(req.body);
        if (!email || !motDePasse || !confirmeMotDePasse || !name || !prenom) return res.status(400).json({ statut: false, message: "les champs sont oblibagtoire" })
    ...
    static async login(req, res) {
        try {
            const { email, motDePasse, confirmeMotDePasse } = req.body;
    ...
    static async logout(req, res) {
        ...
        // Si l'utilisateur n'est pas connecté, renvoie une erreur
        if (!userId) {
        ...
        // Renvoie une réponse de déconnexion
        res.status(204).json({ status: true, message: "Déconnecté avec succès" });
    }
